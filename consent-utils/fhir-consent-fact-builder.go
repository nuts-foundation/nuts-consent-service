...
type FhirConsentFactBuilder struct {
}

func (cl FhirConsentFactBuilder) BuildFact(consentData events.ConsentData) ([]byte, error) {
	fact, err := cl.CreateFhirConsentResource(consentData)
	return []byte(fact), err
}

func (cl FhirConsentFactBuilder) FactFromBytes(bytes []byte) (ConsentFact, error) {
	return FhirConsentFact{payload: bytes}, nil
}

func (cl FhirConsentFactBuilder) CreateFhirConsentResource(data events.ConsentData) (string, error) {
...
func (cl FhirConsentFactBuilder) getVersionID(data events.ConsentData) (uint, error) {
...
func (cl FhirConsentFactBuilder) VerifyFact(consentResource []byte) (bool, error) {
	validationClient := nutsFhirValidation.NewValidatorClient()
	nutsFhirValidation.NewValidatorClient()

	valid, errors, err := validationClient.ValidateAgainstSchema(consentResource)
	if !valid {
